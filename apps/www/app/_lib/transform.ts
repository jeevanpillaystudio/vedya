...
export function transformToCuboidFace(
  x: number,
  y: number,
  L: number,
  W: number,
...
export function generateCuboidPoints(
  L: number,
  W: number,
  H: number,
  resolution: number,
  ...
  for (const face of faces) {
    const xSteps = Math.floor(
      (face === "left" || face === "right" ? H : L) / resolution,
    );
    const ySteps = Math.floor(
      (face === "top" || face === "bottom" ? H : W) / resolution,
    );
  ...
  for (const face of faces) {
    ...
    for (let i = 0; i < xSteps; i++) {
      ...
      for (let j = 0; j < ySteps; j++) {
        const y = j * resolution;
        const [xPrime, yPrime, zPrime] = transformToCuboidFace(
          x,
          y,
          L,
          W,
